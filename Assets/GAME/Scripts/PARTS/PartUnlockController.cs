...
public class PartUnlockController : MonoBehaviour
{
    ...
    public void Refresh()
    {
        ...
        if (!PartUnlocked.Wheels && GameManager.Instance.FlyLength > minFlyToTutorialUnlock)
        {
            PartUnlocked.Wheels = true;
            SetDetails(wheels);
        }
        else if (!PartUnlocked.Wings && GameManager.Instance.FlyLength > minFlyToTutorialUnlock)
        {
        ...
        {
            if (grids.Stats[1].RequireDistance <= Records.Instance.MaxDistance)
            {
    ...
    void SetGrids()
    {
        ...
        for (int i = minLevel; i < count; i++)
        {
            ...
            SetIcon(icon, stat.Sprite, stat.RequireDistance, stat.RequireDistance <= Records.Instance.MaxDistance);
        }
        ...
        float space = 1f / (count - minLevel - 1);
        
        int completed = 0;
        float requireDistance = 0;
        float distance = Records.Instance.MaxDistance;
        ...
        for(int i = minLevel; i < count; i++)
        {
            ...
            {
                requireDistance = Stats[i].RequireDistance - Stats[i - 1].RequireDistance;
                distance -= Stats[i - 1].RequireDistance;
                break;
            }
        ...
        // Debug.Log($"Left distance - {distance}, left require distance - {requireDistance}, space - {space}");
