...
{
    [Inject] public static BuyPart Instance { get; private set; }
    ...
    {
        costText.text = $"{Cost}";
        if (Cost <= Gold.Instance.Value && MergeGrid.FreeCount != 0)
        {
            ChangeObject(true);
        }
        else
    ...
    public void OnButtonClick_Buy()
    {
        ...
        if (Gold.Instance.Value >= Cost)
        {
            Gold.Instance.Minus(Cost);
            Buy();
        }
    }
